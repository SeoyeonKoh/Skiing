...
class OCAtariRAMWrapper(gym.Env):
    def __init__(self, env_name, mode="ram", obs_mode="ori", render_mode="rgb_array", stack_frames=1):
        super().__init__()
        self.env = OCAtari(env_name=env_name, mode=mode, obs_mode=obs_mode, render_mode=render_mode, buffer_window_size=1)
...
env = OCAtariRAMWrapper(env_name="ALE/Skiing-v5", mode="ram", obs_mode="ori", render_mode="rgb_array", stack_frames=1)
